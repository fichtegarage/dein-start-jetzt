...
const faqCategories = [
  ...
  },
  {
    ...
    questions: [
      ...
      {
        id: "erwartung-frueher-aufgehoert",
...
const FAQ = ({ includeIds, hideFooterNote = false }: FAQProps) => {
  const { ref, isVisible } = useScrollAnimation();

  const visibleCategories = includeIds
    ? faqCategories
        .map((category) => ({
          ...category,
          questions: category.questions.filter((item) => includeIds.includes(item.id)),
        }))
